...
const MantraRunnerView: React.FC<MantraRunnerViewProps> = ({
  item,
  isViewOnly,
  initialReps,
  lockCount,
  ...
  // onGoalReached is intentionally NOT wired here. Completion is detected in
  // handleIncrement based on localCount (which always starts at 0 per session),
  // so it fires after exactly selectedTarget taps regardless of accumulated
  // today counts in the engine's cumulative sessionCountRef.
  ...
  const handleIncrement = useCallback(() => {
    ...
    if (mantraRef) {
      ...
      // Completion: based on localCount (starts at 0 per session), not engine's
      // cumulative today count. This ensures exactly selectedTarget taps always
...
export default MantraRunnerView;
