...
const CommunityDetail = () => {
  ...
  const { communityDetail, communityPosts } = useSelector(
  ...
  const translatedCommunityName = community
    ? getTranslatedContent(community, "name")
    : "Community";
  const translatedCommunityDescription = community
    ? getTranslatedContent(community, "description")
    : "";
  const posts = communityPosts.data;
  const loading =
    communityDetail.loading ||
  ...
  const handleLoadMore = () => {
    if (
      !communityPosts.loading &&
      communityPosts.pagination.currentPage <
        communityPosts.pagination.totalPages
    ) {
      dispatch(
        fetchCommunityPosts(
          slug,
          communityPosts.pagination.currentPage + 1,
          sortBy,
          i18n.language,
        ) as any,
      );
            ...
            { text: "Cancel", style: "cancel" },
            {
              ...
              onPress: async () => {
                ...
                if (res.success) {
                  // Refresh posts
                  dispatch(
                    fetchCommunityPosts(slug, 1, sortBy, i18n.language) as any,
                  );
                } else {
  ...
  const renderHeader = () => (
    <View style={styles.communityHeader}>
      <View style={styles.headerContent}>
        <View style={styles.avatarWrapper}>
          <View style={styles.avatarContainer}>
            <Image
              source={
                COMMUNITY_BACKGROUNDS[slug] || {
                  uri: "https://via.placeholder.com/400x150",
                }
              }
              style={styles.avatar}
            />
      ...
      <TouchableOpacity
        ...
        onPress={() =>
          navigation.navigate("CreateSocialPost", { communitySlug: slug })
        }
      >
    ...
    return (
      <View style={styles.aboutContainer}>
        ...
        <Text style={styles.aboutDescription}>
          {translatedCommunityDescription || "No description available."}
        </Text>
        ...
        {rules.map((rule, index) => (
          <View key={index} style={styles.ruleItem}>
            ...
            >
              ...
              <Ionicons
                name={
                  expandedRules.includes(index) ? "chevron-up" : "chevron-down"
                }
                size={20}
                color="#666"
              />
  ...
  return (
    <SafeAreaView style={styles.container}>
      ...
      <Animated.View
        style={[styles.navHeader, { transform: [{ translateY: headerY }] }]}
      >
        <TouchableOpacity
