...
const CreateSocialPost = () => {
  ...
  const initialCommunity =
    editPostData?.community?.slug ||
    editPostData?.community_slug ||
      ...
      const loadPostDetails = async () => {
        ...
        try {
          const res = await api.get(
            `/posts/${editPostData.id}/?lang=${i18n.language}`,
          );
          const fullPost = res.data;
          setTitle(fullPost.title || "");
          setBody(fullPost.content || "");
          setSelectedCommunity(
            fullPost.community?.slug || fullPost.community_slug || "",
          );
  ...
  const handlePost = async () => {
    ...
    try {
      ...
      const videoFile = successfulMedia.find((m) =>
  ...
  const isPostDisabled =
    ...
    !title.trim() ||
    isLoading ||
    mediaFiles.some((m) => m.status === "uploading");
