...
const ExploreCommunities = ({
  ...
  const {
    ...
    loading,
    pagination,
  } = useSelector((state: any) => state.communities);
  ...
  const filteredCommunities = React.useMemo(() => {
    const raw =
      activeTab === "all"
        ? communities
        : (followed_communities?.data || []).map((c: any) => ({
            ...c,
            is_followed: true,
          }));
  ...
  const isLoading =
    activeTab === "all" ? loading : followed_communities.loading;
        ...
        <View style={styles.card}>
          ...
          >
            <Text style={styles.description} numberOfLines={3}>
              {item.description ||
                "Share festival memories, meanings, and ways to keep their sacred essence alive."}
            </Text>
          </TouchableOpacity>
        </View>
      );
    ...
    <View style={styles.container}>
      {isLoading && filteredCommunities.length === 0 ? (
          ...
          ListHeaderComponent={
            <>
              ...
              <Text style={styles.sectionTitle}>
                {activeTab === "all"
                  ? t("community.sections.recommended")
                  : t("community.sections.following")}
              </Text>
            </>
          }
        />
        ...
        <FlatList
          ...
          keyExtractor={(item) =>
            (item.id || item.slug || item.name).toString()
          }
          style={{ flex: 1 }}
          contentContainerStyle={[styles.listContent, { paddingTop: 60 }]}
