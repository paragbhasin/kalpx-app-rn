...
export function WhyThisL2SheetBlock({ screenData, onAction }: Props) {
  ...
  if (!principle) {
    return (
      ...
      >
        <div style={{ width: '100%', maxWidth: 480, background: 'var(--kalpx-card-bg)', borderRadius: '16px 16px 0 0', padding: '24px 20px 40px' }}>
          <button onClick={handleDismiss} style={{ background: 'none', border: 'none', fontSize: 22, color: 'var(--kalpx-text-muted)', cursor: 'pointer' }}>×</button>
    ...
    >
      ...
      >
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 20 }}>
          <p style={{ fontSize: 11, fontWeight: 700, letterSpacing: 2, color: 'var(--kalpx-cta)', textTransform: 'uppercase', margin: 0 }}>
            The Principle
          </p>
          <button
        ...
        {principle.title && (
          <p style={{ fontSize: 20, fontWeight: 700, color: 'var(--kalpx-text)', marginBottom: 10, fontFamily: 'var(--kalpx-font-serif)' }}>
            {principle.title}
          </p>
        )}
        {principle.description && (
          <p style={{ fontSize: 15, color: 'var(--kalpx-text-soft)', lineHeight: 1.7, marginBottom: 20 }}>
            {principle.description}
          </p>
        )}
        {principleId && (
