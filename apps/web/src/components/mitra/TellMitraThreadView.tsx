...
export function TellMitraThreadView({
  ...
  const QUICK_START_CHIPS = [
    {
      label: t("mitra.tellMitra.quickStart.overwhelmed"),
      value: "overwhelmed",
    },
    {
      label: t("mitra.tellMitra.quickStart.needClarity"),
      value: "need_clarity",
    },
    {
      label: t("mitra.tellMitra.quickStart.disconnected"),
      value: "disconnected",
    },
  ...
  const RETURN_CARD_CHIPS: TellMitraFollowupOption[] = [
    ...
    {
      label: t("mitra.tellMitra.returnCard.needClarity"),
      value: "need_clarity",
    },
      ...
      return (
        ...
        >
          ...
          <div
            style={{
              fontFamily:
                "var(--kalpx-font-serif, 'Cormorant Garamond', serif)",
    ...
    if (item.type === "followup_chips") {
      ...
      return (
        <div key={item.id} style={{ marginBottom: 20 }}>
          <div style={{ fontSize: 13, color: "#7B6550", marginBottom: 10 }}>
            {item.prompt}
          </div>
            ...
            {item.options.map((opt) => (
              <button
                ...
                style={{
                  ...
                  cursor:
                    item.disabled || submitting ? "not-allowed" : "pointer",
                  background: item.disabled
                    ? "rgba(240,235,228,0.6)"
                    : CHIP_STYLE.background,
        ...
        <div key={item.id} style={{ marginBottom: 24 }}>
          ...
          >
            ...
            <button
              ...
              style={{ ...GOLD_BTN, marginBottom: 10 }}
            >
              {t("mitra.tellMitra.enterRoom").replace(
                "{{roomLabel}}",
                item.room_label,
              )}
            </button>
              ...
              item.secondary_room_id !== item.room_id && (
                <button
                  onClick={() =>
                    onChipClick(
                      {
                        label: t("mitra.tellMitra.orTryRoom").replace(
                          "{{room}}",
      ...
      return (
        <div key={item.id} style={{ marginBottom: 20 }}>
          ...
          >
            ...
            >
              {t("mitra.tellMitra.returnCard.backFrom").replace(
                "{{roomLabel}}",
                item.room_label,
              )}
            </div>
        ...
        <div key={item.id} style={{ marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {item.next_options.map((opt, i) => (
              ...
              >
                {opt.label} —{" "}
                <span style={{ color: "#9B8B77" }}>{opt.description}</span>
              </button>
            ))}
          </div>
        </div>
    ...
    if (item.type === "safety") {
      return (
        ...
        >
          ...
          <div style={{ fontSize: 16, lineHeight: 1.8, color: "#3B2A1A" }}>
            {item.response_copy}
          </div>
        </div>
      );
  ...
  return (
    ...
    >
      ...
      {conversation.length > 0 && (
        <div
          style={{
            padding: isDesktop ? "16px 28px 0" : "12px 24px 0",
            display: "flex",
            justifyContent: "flex-end",
          }}
        >
      ...
      {/* Scrollable thread area — empty state is vertically centered */}
      <div
      ...
      >
        ...
        >
          {error && (
            <div style={{ fontSize: 13, color: "#e06060", marginBottom: 6 }}>
              {error}
            </div>
          )}
          <div style={{ display: "flex", gap: 10, alignItems: "stretch" }}>
            ...
            <button
              onClick={() => {
                if (!submitting && composerValue.trim())
                  onSubmit(composerValue.trim());
              }}
              ...
              style={{
                ...GOLD_BTN,
                width: "auto",
                padding: isDesktop ? "0 28px" : "0 22px",
                borderRadius: isDesktop ? 16 : 14,
                fontSize: isDesktop ? 15 : 14,
                flexShrink: 0,
                alignSelf: "stretch",
