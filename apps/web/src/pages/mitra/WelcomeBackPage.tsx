...
function parseReentryData(envelope: any, fallbackName?: string): ReentryData {
  ...
  const decisions: string[] = payload.decisions_available ?? [
    "continue",
    "fresh",
  ];
...
export function WelcomeBackPage() {
    ...
    (async () => {
      try {
        ...
        if (viewKey === "onboarding_start") {
          routedRef.current = true;
          navigate(mapJourneyEntryViewPath("onboarding_start"), {
            replace: true,
          });
          return;
        }
  ...
  const isDeepTier =
    reentry.tier === "medium" ||
    reentry.tier === "long" ||
    reentry.tier === "very_long";
  const hasClassicOptions =
    !!continueChip && !!freshChip && reentry.chips.length === 2;

  const freshFirst =
    reentry.fresh_restart_suggested ||
    reentry.primary_recommendation === "fresh";
  ...
  // App parity: when backend sends the standard continue/fresh pair,
  // always render the two-card reentry chooser before any deep-tier variant.
  if (hasClassicOptions) {
    return (
      <MitraMobileShell>
        <div
          style={{
        ...
        >
          ...
          >
            ...
            <img src="/lotus_icon.png" width={20} height={16} />
            <div
              style={{
                flex: 1,
                height: 1,
            ...
            {continueChip && (
              <button
                ...
                style={{
                  ...
                  textAlign: "left",
                  boxShadow: "0 10px 28px rgba(162, 132, 70, 0.08)",
              ...
              >
                <div
                  style={{
                    ...
                    height: 45,
                    borderRadius: 43,
                    background: "rgba(238,229,216,0.45)",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    marginRight: 16,
                ...
                >
                  <Heart size={20} color="#C79A2B" strokeWidth={1.8} />
                </div>
                <div style={{ flex: 1 }}>
                  <p
                    style={{
            ...
            {freshChip && (
              <button
                ...
                style={{
                  ...
                  touchAction: "manipulation",
                  textAlign: "left",
                  boxShadow: "0 10px 28px rgba(162, 132, 70, 0.06)",
                  opacity: submitting ? 0.6 : 1,
                }}
              >
                <div
                ...
                >
                  <Leaf size={20} color="#C79A2B" strokeWidth={1.8} />
                </div>
                <div style={{ flex: 1 }}>
                  <p
                    style={{
        ...
        {!!reentry.earned_context && (
          <div
            style={{
              ...
              borderRadius: 20,
              border: "1px solid rgba(217,194,142,0.4)",
              padding: "16px 20px",
              marginBottom: 8,
              display: "flex",
              flexDirection: "column",
          ...
          >
            {typeof reentry.earned_context.days_practiced === "number" && (
              ...
              >
                ...
                >
                  {reentry.earned_context.days_practiced}
                </span>
              </div>
            )}
            {typeof reentry.cycle_count === "number" &&
              reentry.cycle_count > 0 && (
                <div
                  style={{
                    display: "flex",
                    justifyContent: "space-between",
