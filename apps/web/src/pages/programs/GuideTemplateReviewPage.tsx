...
export function GuideTemplateReviewPage() {
  ...
  const statusLabel = STATUS_LABEL[template.review_status] ?? template.review_status;
  ...
  return (
    <div style={page}>
      <div style={inner}>
        ...
        {(template.review_status === "submitted" || template.review_status === "under_review") && (
          <div style={infoBanner}>
            Your program is with the KalpX team for review. You'll be notified once it's approved.
          </div>
        )}
        ...
        <div style={metaCard}>
          ...
          {template.subtitle && <FieldRow label="Subtitle" value={template.subtitle} />}
        ...
        <div style={{ display: "flex", flexDirection: "column" as const, gap: 16, marginTop: 20 }}>
          {days.map((day) => <DayReviewCard key={day.day_number} day={day} />)}
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={card}>
      ...
      <button
        ...
        style={dayHeaderBtn}
      >
        <span style={dayNumLabel}>Day {day.day_number}{day.theme ? ` — ${day.theme}` : ""}</span>
        <span style={{ fontSize: 14, color: "#B5A08A" }}>{open ? "▲" : "▼"}</span>
      </button>
...
function SlotReview({
  ...
  label: string;
  card?: LibraryMantra | LibrarySankalp | LibraryPractice | LibraryWisdom | null;
  customTitle?: string;
  customBody?: string;
      ...
      {hasLibraryCard ? (
        <div style={selectedCard}>
          <div style={selectedCardTop}>
            <div>
              <p style={selectedCardTitle}>{(card as any).title || (card as any).text || (card as any).item_id}</p>
              {(card as any).category_label && (
                <p style={{ fontSize: 11, color: "#B5A08A", margin: "2px 0 0" }}>
                  {(card as any).category_label}
                </p>
              )}
            </div>
...
function MantraCard({ card }: { card: LibraryMantra }) {
  return (
    <>
      {card.devanagari && <DetailRow label="Devanagari" value={card.devanagari} />}
...
function SankalpCard({ card }: { card: LibrarySankalp }) {
  return (
    <>
      ...
      {card.insight && <DetailRow label="Essence / Insight" value={card.insight} />}
      {card.how_to_live?.length > 0 && <DetailRow label="How to Live" value={card.how_to_live} />}
      {card.benefits?.length > 0 && <DetailRow label="Benefits" value={card.benefits} />}
    </>
  );
}
...
function WisdomCard({ card }: { card: LibraryWisdom }) {
  return (
    <>
      ...
      {card.explanation && <DetailRow label="Explanation" value={card.explanation} />}
      {card.source_title && <DetailRow label="Source" value={card.source_title + (card.source_ref ? ` (${card.source_ref})` : "")} />}
    </>
  );
}
    ...
    <div style={detailRowStyle}>
      ...
      {Array.isArray(value) ? (
        <ul style={detailListStyle}>
          {value.map((v, i) => <li key={i} style={detailListItem}>{v}</li>)}
        </ul>
      ) : (
...
function FieldRow({ label, value, multiline }: { label: string; value: string; multiline?: boolean }) {
  return (
    <div style={{ marginBottom: 12 }}>
      ...
      {multiline ? (
        <textarea disabled value={value} rows={3}
          style={{ ...disabledInput, resize: "none", fontFamily: "inherit" }} />
      ) : (
...
const backBtn: React.CSSProperties = {
  ...
  color: "#8B6F4E", fontSize: 13, padding: 0, marginBottom: 20,
};
...
const eyebrow: React.CSSProperties = {
  fontSize: 10, letterSpacing: "1.2px", color: "#8B6F4E",
  textTransform: "uppercase" as const, marginBottom: 4, margin: "0 0 4px",
};
const heading: React.CSSProperties = { fontSize: 22, fontWeight: 800, color: "#432104", margin: "0 0 8px" };
const infoBanner: React.CSSProperties = {
  background: "#EFF6FF", border: "1px solid #BFDBFE",
  borderRadius: 8, padding: "10px 16px", fontSize: 13, color: "#1d4ed8", marginBottom: 20,
};
const warnBanner: React.CSSProperties = {
  background: "#FFFBEB", border: "1px solid #FDE68A",
  borderRadius: 8, padding: "10px 16px", fontSize: 13, color: "#92400e", marginBottom: 20,
};
const editBtn: React.CSSProperties = {
  padding: "8px 18px", background: "#C99317", color: "#fff",
  border: "none", borderRadius: 8, cursor: "pointer", fontWeight: 700, fontSize: 13,
  flexShrink: 0,
};
const metaCard: React.CSSProperties = {
  background: "#fff", border: "1px solid #E8DECE",
...
const dayNumLabel: React.CSSProperties = {
  fontSize: 14, fontWeight: 800, color: "#432104",
};
const sectionLabelStyle: React.CSSProperties = {
  fontSize: 10, fontWeight: 700, color: "#B5A08A",
  textTransform: "uppercase" as const, letterSpacing: "0.08em",
  margin: "14px 0 4px",
};
const fieldLabel: React.CSSProperties = {
  fontSize: 11, fontWeight: 600, color: "#B5A08A",
  margin: "0 0 4px",
};
const disabledInput: React.CSSProperties = {
  width: "100%", padding: "9px 12px", border: "1px solid #E8DECE",
  borderRadius: 8, fontSize: 13, color: "#432104",
  background: "#F7F3ED", cursor: "not-allowed",
  boxSizing: "border-box" as const,
};
const twoCol: React.CSSProperties = { display: "flex", gap: 12, marginBottom: 8 };
const emptySlot: React.CSSProperties = {
  padding: "10px 14px", border: "1px dashed #E8DECE",
  borderRadius: 8, fontSize: 13, color: "#C5B69A", background: "#FDFAF6",
};
const selectedCard: React.CSSProperties = {
  border: "1px solid #C99317", borderRadius: 10,
  padding: "10px 14px", background: "#FFFDF6",
};
...
const selectedCardTitle: React.CSSProperties = {
  fontSize: 14, fontWeight: 700, color: "#432104", margin: 0,
};
const detailToggleBtn: React.CSSProperties = {
  background: "none", border: "none", cursor: "pointer",
  color: "#C99317", fontSize: 12, fontWeight: 600, padding: 0, flexShrink: 0,
};
...
const detailValueStyle: React.CSSProperties = {
  fontSize: 13, color: "#432104", flex: 1, lineHeight: 1.5,
};
const detailListStyle: React.CSSProperties = {
  margin: "0", padding: "0 0 0 16px", flex: 1,
};
const detailListItem: React.CSSProperties = {
  fontSize: 13, color: "#432104", lineHeight: 1.5, marginBottom: 2,
};
const customContentBox: React.CSSProperties = {
  border: "1px solid #E8DECE", borderRadius: 8,
  padding: "10px 14px", background: "#FDFAF6",
};
const customTitleStyle: React.CSSProperties = {
  fontSize: 13, fontWeight: 700, color: "#432104", margin: "0 0 4px",
};
