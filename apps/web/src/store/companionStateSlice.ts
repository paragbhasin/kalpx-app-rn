...
export const persistCompanionState = createAsyncThunk(
  ...
  async (_, { getState }) => {
    ...
    const payload = JSON.stringify({ _version: COMPANION_STATE_VERSION, data: state });
    localStorage.setItem(COMPANION_STATE_STORAGE_KEY, payload);
  },
);
...
export const restoreCompanionState = createAsyncThunk(
  ...
  async () => {
    const raw = localStorage.getItem(COMPANION_STATE_STORAGE_KEY);
    if (!raw) return null;
    try {
      const parsed = JSON.parse(raw);
      if (parsed?._version !== COMPANION_STATE_VERSION) return null;
...
const companionStateSlice = createSlice({
  ...
  reducers: {
    ...
    upsertDissonanceThread(
      ...
      if (idx === -1) {
        state.dissonance_threads.push({ id, first_seen_at: Date.now(), ...patch });
      } else {
        state.dissonance_threads[idx] = { ...state.dissonance_threads[idx], ...patch };
      }
      state.last_updated = Date.now();
    },
    removeDissonanceThread(state, action: PayloadAction<string>) {
      state.dissonance_threads = state.dissonance_threads.filter((t) => t.id !== action.payload);
      state.last_updated = Date.now();
    },
  ...
  extraReducers: (builder) => {
    builder
      .addCase(fetchCompanionState.fulfilled, (state, action) => {
        ...
        if (Array.isArray(data.dissonance_threads)) state.dissonance_threads = data.dissonance_threads;
        if ('life_context' in data) state.life_context = data.life_context ?? null;
        state.last_updated = Date.now();
      })
