...
const LibrarySearchModal: React.FC<LibrarySearchModalProps> = ({ isVisible, onClose, onItemAdded, journeyId }) => {
  ...
  const handleAddItem = async (item: any) => {
    ...
    try {
      ...
      setResults(prev => prev.map(r => 
        (r.itemId === itemId || (r as any).item_id === itemId) ? { ...r, alreadyAdded: true } : r
      ));
  ...
  const getLevelStyle = (item: any) => {
    const level = String(item.level || '').trim().toLowerCase();
    if (level === 'intermediate') return { color: '#8a5a1d', backgroundColor: 'rgba(212, 168, 76, 0.16)' };
    if (level === 'advanced') return { color: '#7a3358', backgroundColor: 'rgba(164, 97, 137, 0.14)' };
      ...
      <SafeAreaView style={styles.container}>
        ...
        >
          ...
          <Text style={styles.helpText}>
            Search for mantras, sankalps, or practices to add to your daily journey.
          </Text>

          {/* Search Input */}
          <View style={styles.searchContainer}>
            <Ionicons name="search" size={20} color="#8f877f" style={styles.searchIcon} />
            <TextInput
          ...
          {loading ? (
            ...
            <FlatList
              data={results}
              keyExtractor={(item, idx) => (item.itemId || (item as any).item_id || '') + idx}
              ...
              renderItem={({ item }: { item: any }) => (
                <View style={[styles.itemCard, (item.alreadyInCore || item.alreadyAdded) && styles.disabledCard]}>
                  <View style={styles.itemInfo}>
                    <View style={styles.badgeRow}>
                      <Text style={styles.itemTypeBadge}>{item._type || item.itemType}</Text>
