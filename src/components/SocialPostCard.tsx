...
const SocialPostCard: React.FC<SocialPostCardProps> = ({
  ...
  const translatedCommunityName =
    getTranslatedContent(post, "community_name") || post.community?.name;

  const translatedLinkedItemName = useMemo(() => {
    if (!post.linked_item || !post.linked_item.id)
      return post.linked_item?.name || "";
  ...
  const [selectedLinkedPractice, setSelectedLinkedPractice] =
    useState<any>(null);
  const [linkedCardType, setLinkedCardType] = useState<
  ...
  const timeAgo = post.created_at
    ? moment(post.created_at).locale(currentLang).fromNow()
    : "";
  ...
  const getImagesData = () => {
    ...
    if (
      ...
      post.resolved_slide_layouts.length > 0
    ) {
      return post.resolved_slide_layouts;
    }
    if (post.slide_layouts && post.slide_layouts.length > 0)
      return post.slide_layouts;
    if (post.slides && post.slides.length > 0) return post.slides;
    if (post.ai_output?.slides && post.ai_output.slides.length > 0)
      return post.ai_output.slides;
  ...
  const renderCarouselItem = ({
    ...
    const imageUrl =
      item.image_url || item.image || (typeof item === "string" ? item : null);
    const isVideo =
    ...
    >
      ...
      <View style={[styles.header, { zIndex: showMenu ? 1001 : 1 }]}>
        <TouchableOpacity
          onPress={() => {
            const slug =
              post.community?.slug || post.community_slug || post.slug;
            if (slug) {
            ...
            <ImageBackground
              source={
                ...
                (typeof post.community === "string"
                  ? COMMUNITY_BACKGROUNDS[post.community]
                  : null) ||
            ...
            >
              <Text
                style={[
                  ...
                  post.is_joined && styles.joinedText,
                ]}
              >
                ...
                <>
                  ...
                  >
                    ...
                    <Text style={styles.menuItemText}>
                      {t("community.postMenu.edit") || "Edit Post"}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                  ...
                  >
                    ...
                    <Text style={[styles.menuItemText, { color: "#FF3B30" }]}>
                      {t("community.postMenu.delete") || "Delete Post"}
                    </Text>
                  </TouchableOpacity>
                </>
              ) : (
                <>
                  ...
                  >
                    <Ionicons
                      ...
                      size={20}
                      color={post.is_saved ? "#D69E2E" : "#333"}
                    />
                    <Text
                      style={[
                        styles.menuItemText,
                        post.is_saved && { color: "#D69E2E" },
                      ]}
                    >
                      {post.is_saved
                        ? t("community.postMenu.unsave")
                        : t("community.postMenu.save")}
                    </Text>
                  </TouchableOpacity>
                  <TouchableOpacity
                    style={styles.menuItem}
      ...
      {imagesData.length > 0 && (
        <View>
          ...
          {/* Dot Indicators and Counter below the post */}
          {imagesData.length > 1 && (
            <View style={styles.paginationCounterWrapper}>
          ...
          {shouldTruncate && (
            <TouchableOpacity onPress={() => setIsExpanded(!isExpanded)}>
              <Text style={styles.showMore}>
                {isExpanded
                  ? t("community.post.showLess")
                  : t("community.post.showMore")}
              </Text>
            </TouchableOpacity>
          )}
        </View>
      ...
      <View style={styles.footer}>
        <View style={styles.leftActions}>
          ...
          <TouchableOpacity
            ...
            onPress={() =>
              handleProtectedAction(() => onComment?.(), {
        ...
        <TouchableOpacity
          ...
          onPress={() =>
            handleProtectedAction(() => onAskQuestion?.(), {
