...
const InsightSummaryContainer: React.FC<InsightSummaryContainerProps> = ({
  schema,
}) => {
  const { isLoading: isFetching, aiReasoning: globalAiReasoning } = useSelector(
    (state: RootState) => state.mitra,
  );
  ...
  }, [step, updateBackground, updateHeaderHidden]);

  const activeFocus =
    screenState.active_focus || screenState.scan_focus || "career";
  ...
  const getCategoryData = (focus: string) => {
    ...
    if (disciplineState) {
      const choiceBlock = (disciplineState.blocks || []).find(
  ...
  const subCatData = useMemo(
    () => getSubCategoryData(activeFocus, subFocus),
    [activeFocus, subFocus],
  );
  ...
  const experienceLabel =
    currentConfig.experience_label || "WITHIN THIS, YOU'RE EXPERIENCING:";
  const experienceExplanation = (subCatData?.explanation || "").replace(
    /<b>|<\/b>/g,
    "",
  );
  ...
  const handleNext = () => {
    if (step === 0) {
      ...
      const target = schema.on_complete?.target || {
  ...
  const renderStep0 = () => (
    <View style={styles.stepContainer}>
      {(() => {
        ...
        if (
          typeof SevenDaysLotus === "function" ||
          (typeof SevenDaysLotus === "object" && SevenDaysLotus !== null)
        ) {
      ...
      <View style={styles.understandingContent}>
        <View style={styles.glassPathCard}>
          <Text style={styles.sectionTitle}>
            {currentConfig.chosen_label}{" "}
            <Text style={styles.highlight}>{catData?.title}</Text>.
          </Text>
          {!!topDescription && (
            <Text style={styles.introP}>{topDescription}</Text>
          )}
  ...
  const renderStep2 = () => {
    const practiceBlocks = (schema.blocks || []).filter(
      (b: any) => b.type === "practice_card",
    );
    const summaryText =
      resolveTemplate(screenState.analysis_insight) ||
      globalAiReasoning ||
          ...
          >
            ...
            >
              <Text style={styles.buttonText}>
                {currentConfig.button_label}
              </Text>
            </LinearGradient>
          </TouchableOpacity>
          <Text style={styles.footerNoteKalpx}>
            {currentConfig.footer_note}
          </Text>
        </View>
      </View>
    );
...
const styles = StyleSheet.create({
  ...
  step2Card: {
    ...
    borderWidth: 2,
    borderColor: "#E8C587",
    // backgroundColor: "rgba(248, 240, 226, 0.5)",
    shadowColor: "#d7a64a",
    shadowOffset: { width: 0, height: 8 },
    shadowOpacity: 0.15,
    shadowRadius: 16,
