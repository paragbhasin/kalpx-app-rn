...
`src/extensions/moments/gratitude_signal_card/index.tsx`
...
```javascript
{ type: "gratitude_signal_card" },
```

(Backend gates visibility via `screenData.gratitude_card`. You do NOT need to add any `if` in the RN code — the block itself checks `screenData.gratitude_card` and renders null when absent.)
...
```typescript
case "gratitude_signal_card":
  return <GratitudeSignalCard block={block} />;
```
...
Expected: Warm card reflecting the gratitude signal back.

### Option B — Real user flow (the test that matters)
A user never types a URL. Test the tap path from cold launch:

1. Log in as a user whose recent reflections show gratitude/joy signals.
2. If `gratitude_card` is returned, the card renders on the dashboard.
...
- ONE commit per moment. Commit message: `feat(moment): wire gratitude_signal_card`
