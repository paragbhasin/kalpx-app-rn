// Dashboard block (card): GratitudeSignalCard
...
const GratitudeSignalCard: React.FC<Props> = ({ block, screenData }) => {
  const payload = block?.payload ?? screenData?.gratitude_signal ?? null;
  if (!payload) return null;
  const title = payload.title ?? payload.headline ?? "";
  const body = payload.body ?? payload.message ?? "";
  return (
    <View style={styles.card} accessibilityLabel="gratitude_signal_card">
...
export default GratitudeSignalCard;
