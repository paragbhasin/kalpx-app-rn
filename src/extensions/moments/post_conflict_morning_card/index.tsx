// Dashboard block (card): PostConflictMorningCard
...
const PostConflictMorningCard: React.FC<Props> = ({ block, screenData }) => {
  const payload = block?.payload ?? screenData?.post_conflict_morning ?? null;
  if (!payload) return null;
  const title = payload.title ?? payload.headline ?? "";
  const body = payload.body ?? payload.message ?? "";
  return (
    <View style={styles.card} accessibilityLabel="post_conflict_morning_card">
...
export default PostConflictMorningCard;
