// Dashboard block (card): SeasonSignalCard
...
const SeasonSignalCard: React.FC<Props> = ({ block, screenData }) => {
  const payload = block?.payload ?? screenData?.season_signal ?? null;
  if (!payload) return null;
  const title = payload.title ?? payload.headline ?? "";
  const body = payload.body ?? payload.message ?? "";
  return (
    <View style={styles.card} accessibilityLabel="season_signal_card">
...
export default SeasonSignalCard;
