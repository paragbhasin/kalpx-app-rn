...
`src/extensions/moments/sound_bridge/index.tsx`
...
```javascript
support_trigger: { sound_bridge: { blocks: [{ type: "sound_bridge_transient" }] } },
```
...
```typescript
import SoundBridgeTransient from "../extensions/moments/sound_bridge";
```
...
case "sound_bridge_transient":
  return <SoundBridgeTransient block={block} />;
```
...
```bash
xcrun simctl openurl booted kalpx://mitra/support_trigger/sound_bridge
```

Expected: Transient screen with ambient audio then auto-advance.

### Option B — Real user flow (the test that matters)
A user never types a URL. Test the tap path from cold launch:

1. On the dashboard, tap 'I feel triggered' (or similar support entry).
2. In the feeling picker, select any trigger feeling.
3. Read the advice reveal.
4. Expected next: sound bridge transient plays (8-12s), then auto-advances to the practice runner.
...
- ONE commit per moment. Commit message: `feat(moment): wire sound_bridge`
