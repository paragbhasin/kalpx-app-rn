    ...
    <SafeAreaView style={{ flex: 1, backgroundColor: Colors.Colors.white }}>
      ...
      >
        ...
        >
          ...
          <TextComponent type="streakSadanaText" style={{ textAlign: "center", marginHorizontal: 12 }}>{t("sadanaTracker.setupInstruction")}</TextComponent>
          <View
            style={{
              flexDirection: "row",
              alignItems: "center",
          ...
          {paginatedPractices.map((practice: any) => {
            ...
            const translated = getTranslatedPractice(practice, t);
            const displayName = translated.name || practice.name || practice.title || practice.text || "";
            const displayDescription = translated.desc || practice.description || practice.meaning || "";
      ...
      {practice.tags.map((tag, index) => (
        ...
        >
          ...
          >
            ...
            >
              {t("sanatanPractice.addSelected")}
              {/* ({selectedPractices.length}) */}
            </TextComponent>
          </TouchableOpacity>
          <TextComponent type="streakText" style={{ color: Colors.Colors.BLACK, textAlign: "center", marginTop: 4 }}>{t("sanatanPractice.nextStepInstruction")}</TextComponent>
        </View>
        <LoadingOverlay visible={loading} text={t("sanatanPractice.saving")} />
      </ImageBackground>
    </SafeAreaView>
  );
};
